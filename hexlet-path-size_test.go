...
func TestGetSize(t *testing.T) {
	...
	res, _ := GetPathSize(fixturePath, false, false, false)
...
func TestGetSizeError(t *testing.T) {
	fixturePath := ""
	_, err := GetPathSize(fixturePath, false, false, false)
...
func TestHumanSize(t *testing.T) {
	cwd, _ := os.Getwd()
	fixturePath := cwd + "/testdata/fixture_1kb.png"
	res, _ := GetPathSize(fixturePath, false, true, false)
...
func TestIncludeHiddens(t *testing.T) {
	cwd, _ := os.Getwd()
	fixturePath := cwd + "/testdata/fixture_directory_with_hidden"
	res, _ := GetPathSize(fixturePath, false, false, true)
...
func TestWithRecursive(t *testing.T) {
	cwd, _ := os.Getwd()
	fixturePath := cwd + "/testdata"
	res, _ := GetPathSize(fixturePath, true, false, false)
